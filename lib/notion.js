...
// Shared between api/prompts.js's create path and api/prompts/[id]/edit.js
// -- centralized here (rather than duplicated as a local constant in
// each file, which is how it originally lived only in api/prompts.js)
// so both stay in sync automatically if this limit is ever revisited.
// Unicode code points, checked BEFORE any chunking is ever attempted --
// this is the aggregate limit, not a per-chunk one.
...
// Accepts exactly two UTC notations: this app's own write form ("Z",
// via toISOString()) and Notion's real read-back form ("+00:00") --
// confirmed live 2026-08-15, the first real production write after
// deployment: Notion's API echoed a date this app wrote as
// "...000Z" back as "...000+00:00". Both represent the identical UTC
// instant; deliberately NOT widened to accept an arbitrary +HH:MM
// offset, which could represent a genuinely different wall-clock
// instant if ever compared as a raw string elsewhere. This was a
// disclosed, deliberately-not-fixed-blind uncertainty throughout the
// whole diff-review loop (rounds 6 and 8 both declined loosening this
// without live evidence, to avoid reintroducing the rollover-date-
// acceptance bug round 6 originally fixed) -- this is that live
// evidence, and the fix is scoped to exactly the confirmed real case.
...
function isValidIso8601(str) {
  ...
  // the regex + a bare Date.parse. Normalize a trailing "+00:00" to
  // "Z" before comparing -- toISOString() only ever emits "Z", so an
  // un-normalized "+00:00" input would otherwise always fail this
  // check even when genuinely valid.
  const normalized = str.endsWith("+00:00") ? str.slice(0, -6) + "Z" : str;
...
// Compares two already-isValidIso8601-passed strings for representing
// the SAME instant, not for being the SAME STRING. Needed because
// isValidIso8601 deliberately accepts two different valid notations
// for the same UTC instant ("Z" and "+00:00", see its own comment) --
// exact string equality between them would be a false negative even
// though the underlying data is identical. Confirmed live 2026-08-15:
// a genuine, successful Notion PATCH was reported to the user as
// "tracking failed" because the app sent Last Used in "Z" form but
// Notion's response echoed it back in "+00:00" form -- the write had
// already succeeded (confirmed via a manual page refresh showing the
// real incremented count), only the app's own post-hoc confirmation
// check was comparing the wrong thing. Deliberately requires BOTH
// inputs to already be isValidIso8601-valid before calling this --
// it does not itself re-validate format, only compares instants.
function sameInstant(a, b) {
  return new Date(a).getTime() === new Date(b).getTime();
}

// Confirmed live 2026-08-15, immediately after deploying the sameInstant
// fix above: Notion's "date" property type stores at MINUTE granularity
// only. Writing "...T01:52:37.432Z" via PATCH and reading the SAME
// property back afterward returned "...T01:52:00.000+00:00" -- seconds
// and milliseconds silently zeroed, on every real write observed (a
// dedicated live database query, not inference), including cases where
// the same page's Count had genuinely incremented multiple times, proving
// the underlying writes themselves were succeeding while sameInstant()'s
// exact-millisecond comparison could still never pass -- the +00:00-vs-Z
// notation fix (sameInstant, above) was necessary but not sufficient,
// since Notion ALSO discards precision below the minute regardless of
// notation. This is a genuine capability limit of Notion's own API, not
// a bug in this app or in sameInstant() itself -- sameInstant() remains
// exact and correct for anything that hasn't round-tripped through
// Notion's storage. sameMinute() floors both sides to the minute before
// comparing, matching exactly what a Notion round-trip can actually
// confirm. Safe to use for copy.js's post-PATCH confirmation
// specifically: by the point this runs, patchedPage has already been
// confirmed to be the same page (id + database membership) as the one
// just written, and Count (an exact integer, never rounded) is checked
// alongside this -- Count already proves the response reflects THIS
// request's own effect, not a stale/unrelated one, so flooring the
// timestamp comparison doesn't weaken what this check actually protects
// against.
function sameMinute(a, b) {
  const floorToMinute = (ms) => Math.floor(ms / 60000) * 60000;
  return floorToMinute(new Date(a).getTime()) === floorToMinute(new Date(b).getTime());
}
...
// api/prompts/[id]/edit.js's own payload builder -- updates only
// Prompt Text / Normalized Text / Normalized Hash (the three fields
// that must always stay mutually consistent with each other, matching
// isValidPromptRecordShape's own recompute-and-compare check). Count,
// Last Used, and Created are DELIBERATELY ABSENT from this payload --
// Notion's partial-update semantics only touch properties actually
// present in a PATCH request, so omitting them here isn't just
// documentation of intent, it's the actual mechanism that leaves them
// untouched. Mirrors buildCreatePayload's exact same normalize/hash
// logic (trim, lowercase, sha256) so the two paths can never produce
// mutually-inconsistent Normalized Text/Hash values for what is
// otherwise identical input.
function buildEditUpdatePayload(text) {
  const normalizedText = text.trim().toLowerCase();
  const hash = sha256Hex(normalizedText);
  return {
    properties: {
...
function isValidPromptRecordShape(page) {
  ...
  // POST /api/prompts requires trimmed.length > 0 — a record whose own
  // normalized text is empty could never have been created THROUGH
  // this app (only by a manual/external edit directly in Notion), and
  // would violate the "every prompt has real content" assumption every
  // other part of this app makes (library previews, search matching,
  // copy-to-clipboard). An internally CONSISTENT empty record (empty
  // text + the correct hash of an empty string) would otherwise still
  // pass every other check here (diff-review round 8 P2 finding).
  if (recomputedNormalizedText.length === 0) return false;
