...
// at 2000 characters. A naive index-based `.slice()` can split a UTF-16
// surrogate pair (e.g. an emoji) across two chunks, corrupting the
// reconstructed text and silently breaking the hash round-trip check
// below (round 9 P1 finding). `Array.from(text)` iterates by Unicode
// code point, never splitting a surrogate pair.
function chunkRichText(text, maxLen = 2000) {
  ...
  // A genuinely empty string still needs one (empty) chunk, since Notion
  // rich_text/title arrays require at least one element.
...
// canonical date form — a null/absent Last Used is explicitly VALID on
// a record that's never been copied (round 9 P2 — Last Used is genuinely
// unset until the first copy-click, per the BUILD SPEC, not a defect).
function isValidPromptRecordShape(page) {
  ...
  // Last Used: valid if absent (never copied), or if present and a
  // valid canonical date.
  if (lastUsedProp && lastUsedProp.date) {
    if (!isValidIso8601(lastUsedProp.date.start)) return false;
  }
